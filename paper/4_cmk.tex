...
\begin{table*}[h]
\centering
\caption{\textbf{Derivation of CKM Matrix Elements.} 
Comparison of geometric predictions (System III) against PDG 2024 global fit values. 
The upper section lists the calculated matrix elements. The lower section defines the \textbf{Geometric Basis} ($s_{ij} = \sin \theta_{ij}, c_{ij} = \cos \theta_{ij}$) derived directly from the lattice invariants. Note that the suppression of $V_{td}$ and $V_{ts}$ arises from \textbf{Geometric Interference} between these basis vectors, not parameter tuning.}
...
$u \to s$ & $V_{us}$ & Cabibbo Aperture ($s_{12}c_{13}$) & \textbf{\CabibboSineVal} & \CabibboSineExp & $-\CabibboSineSig\sigma$ \\
$u \to b$ & $V_{ub}$ & Quantum Tunneling ($s_{13}e^{-i\delta}$) & \textbf{\VubVal} & \VubExp & $-\VubSig\sigma$ \\
\midrule
...
$c \to b$ & $V_{cb}$ & Dimensional Shift ($s_{23}c_{13}$) & \textbf{\VcbVal} & \VcbExp & $+\VcbSig\sigma$ \\
\midrule
...
\multicolumn{2}{l}{$s_{12}$ (Solar/Cabibbo)} & $\CabibboSineEq$ & \CabibboSineVal & Interface Aperture & \\
\multicolumn{2}{l}{$s_{13}$ (Reactor)} & $\VubEq$ & \VubVal & Quantum Tunneling & \\
\multicolumn{2}{l}{$s_{23}$ (Atmospheric)} & $\VcbEq$ & \VcbVal & Dimensional Drag & \\
\multicolumn{2}{l}{$\delta$ (CP Phase)} & $\CPPhaseEq$ & $\CPPhaseVal$ & Chiral Diode Offset & \\
\bottomrule
\end{tabular}
\end{table*}
