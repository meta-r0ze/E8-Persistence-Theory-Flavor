...
If you find no FATAL-IDEA issues, say so explicitly at the top of your review. 0-100 would a physicist want to read this in its current form and why?  Would about the series?  

- The abstract should answer 'what', the introduction should answer 'why', the conclusion answers 'so what?'
- While we discuss terms like (tunneling penalties, complex phase vectors, amplitudes) when dicussing classic terms we shouldn't use it to describe any geometric behavior.
- Derivations should rely on the geometry. Explaining the 'why' the equation is that way and only stick to justifyable geometric answers even if the sigma is large. Repeating the formula in english isn't good enough and should be flagged as numerology with appropriate correction.
- Understate rather than overstate by default.
