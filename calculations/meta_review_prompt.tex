We are working on this paper which build on 3 previous IE papers.  Below is the general review guidelines:
...
- While we discuss terms like (tunneling penalties, complex phase vectors, amplitudes) when discussing classic terms we shouldn't use it to describe any geometric behavior.
- Derivations should rely on the geometry. Explaining the 'why' the equation is that way and only stick to justifiable geometric answers even if the sigma is large.
- Simply repeating the formula in english isn't good enough and should be flagged as numerology with appropriate correction and a derivation.
